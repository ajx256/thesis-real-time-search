...
struct DFSCsernaBackupPembertonBelief
{
	...
	void generateTopLevelActions(Node* start, int lookahead, ResultContainer& res)
	{
		...
		for (State child : children)
		{
			...
			//open.push(childNode);
			childNode->distribution = DiscreteDistribution(100, childNode->getGValue(), (int)childNode->getD(), domain.getBranchingFactor());
	...
	void kBestDecision()
	{
		...
		for (TopLevelAction& tla : topLevelActions)
		{
			...
			while (i < k && !tla.topLevelOpen.empty())
			{
				...
				best->distribution = DiscreteDistribution(100, best->getGValue(), (int)best->getD(), domain.getBranchingFactor());
