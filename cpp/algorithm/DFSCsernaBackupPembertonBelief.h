...
template <class D>
struct DFSCsernaBackupPembertonBelief
{
	...
	DFSCsernaBackupPembertonBelief(D& domain) : domain(domain) {}

	~DFSCsernaBackupPembertonBelief()
	{
	...
	void generateTopLevelActions(Node* start, int lookahead, ResultContainer& res)
	{
		...
		for (State child : children)
		{
			...
			//open.push(childNode);
			childNode->distribution = DiscreteDistribution(100, childNode->getGValue(), childNode->getD(), domain.getBranchingFactor());
	...
	void explore(Node* cur, int curDepth, int maxDepth, ResultContainer& res)
	{
		...
		if (curDepth > maxDepth || domain.isGoal(cur->getState()))
		{
			...
			if (topLevelActions[cur->getOwningTLA()].expectedMinimumPathCost == -1)
			{
				topLevelActions[cur->getOwningTLA()].distribution = cur->distribution;
				topLevelActions[cur->getOwningTLA()].expectedMinimumPathCost = topLevelActions[cur->getOwningTLA()].distribution.expectedCost();
			}
			else
			{
				topLevelActions[cur->getOwningTLA()].distribution = topLevelActions[cur->getOwningTLA()].distribution * cur->distribution;
				topLevelActions[cur->getOwningTLA()].expectedMinimumPathCost = topLevelActions[cur->getOwningTLA()].distribution.expectedCost();
			}
		}
		else
