...
struct KBest
{
	...
	void generateTopLevelActions(Node* start, ResultContainer& res)
	{
		...
		for (State child : children)
		{
			...
			open.push(childNode);
			childNode->distribution = DiscreteDistribution(100, childNode->getFHatValue(), childNode->getD() / (domain.getBranchingFactor() + 1));
	...
	void kBestDecision()
	{
		...
		for (TopLevelAction& tla : topLevelActions)
		{
			...
			while (i < k && !tla.topLevelOpen.empty())
			{
				...
				best->distribution = DiscreteDistribution(100, best->getFHatValue(), best->getD() / (domain.getBranchingFactor() + 1));
	...
	string lastIncrementalDecision()
	{
		...
		while (1)
		{
			...
			for (State child : children)
			{
				...
				tla.topLevelOpen.push(childNode);
				open.push(childNode);
				childNode->distribution = DiscreteDistribution(100, childNode->getGValue(), (int)childNode->getD());
				// Push this node onto open
				openUclosed[childNode->getState().hash()].push_back(childNode);
				// Add this top level action to the list
				topLevelActions.push_back(tla);
	...
	void pembertonKBestDecision()
	{
		...
		for (TopLevelAction& tla : topLevelActions)
		{
			...
			while (i < k && !tla.topLevelOpen.empty())
			{
				...
				// Make this node's PDF a discrete distribution...
				best->distribution = DiscreteDistribution(100, best->getGValue(), (int)best->getD());
