...
if (($# < 3))
then
  echo "./backupTestHarnessAS.sh <# of processes> <Domain Type> <Lookahead value 1> <optional: additional lookahead values>"
...
then
  for lookahead in "${@:3}"
  do
    mkdir ../results/backupTests/AS/SlidingTilePuzzle
    instance=0
    ...
    do
	  ...
	  if [ -f ../results/backupTests/AS/SlidingTilePuzzle/LA${lookahead}-${instance}.json ]
	  then 
	    let instance++
	  else
	    ./../backupTestsAS.sh ${domainType} ${lookahead} ../results/backupTests/AS/SlidingTilePuzzle/LA${lookahead}-${instance}.json < ${file} &
	    let instance++
        let numProcs++
	  fi
    done
